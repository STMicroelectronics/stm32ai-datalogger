...
#define MAX_SEND_PACKET_SIZE 64 //Size in which the data will be splitted to be sent via "FillTxDataBuffer", also defines the internal USB buffers
...
static int8_t WCID_STREAMING_Itf_Init(void)
{
  USBD_WCID_STREAMING_SetRxDataBuffer(&hUsbDeviceHS, (uint8_t *)USB_RxBuffer);
...
void start_streaming()
{
  if(streaming_started != 1)
  {
    for(int i = 0; i < N_IN_ENDPOINTS; i++)
    {
      ...
      USBD_WCID_STREAMING_SetTxDataBuffer(&hUsbDeviceHS, i, TxBuffer[i], MAX_SEND_PACKET_SIZE);
      USBD_WCID_STREAMING_CleanTxDataBuffer(&hUsbDeviceHS, i);
    }
    USBD_WCID_STREAMING_StartStreaming(&hUsbDeviceHS);
    streaming_started = 1;
  } 
}
...
uint32_t usb_wcid_send_function(uint8_t *data_ptr, uint32_t data_size)
{
  ...
  USBD_WCID_STREAMING_HandleTypeDef   *hwcid = (USBD_WCID_STREAMING_HandleTypeDef*) hUsbDeviceHS.pClassData;
  ...
  while(bytesLeft > 0)
  {
    if(bytesLeft - MAX_SEND_PACKET_SIZE <= 0)
    {
      ...
      {
        USBD_WCID_STREAMING_FillTxDataBuffer(&hUsbDeviceHS, channel, (uint8_t*)data_ptr+(data_size - bytesLeft), bytesLeft);
        bytesLeft = 0;
      }
    }
    else
    {
      ...
      if(TX_States[channel] == 0 && TxBuffStatus[channel] == 0)
      {
        USBD_WCID_STREAMING_FillTxDataBuffer(&hUsbDeviceHS, channel, (uint8_t*)data_ptr+(data_size - bytesLeft), MAX_SEND_PACKET_SIZE);     
